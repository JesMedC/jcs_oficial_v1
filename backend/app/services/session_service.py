...
``ASIA | LONDON | NEW_YORK | SYDNEY`` based on the LOCAL hour after
...
The four UTC windows in the spec:
    ...
    NEW_YORK   12:00–17:00 UTC
    SYDNEY     17:00–24:00 UTC

Critical edge case (per design §9 / REQ-DISC-003): a UTC ``02:00``
falls inside ``SYDNEY`` for ``America/Buenos_Aires`` because it's
...
def session_for_timestamp(ts: datetime, tz: str) -> Band:
    ...
    Returns the matching band. Falls through to ``SYDNEY`` for the
    23:00..23:59 bucket (SYDNEY owns hours 17..23 inclusive — hour 24
